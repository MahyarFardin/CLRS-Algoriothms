...
public class redBlackTree {
    protected class RedBlackNode{
        ...
        private int data, color, size, max;
    ...
    public RedBlackNode SELECT(RedBlackNode inputNode, int i){
        int temp = inputNode.getLeftChild().getSize() + 1;
        if(i == temp){
            return inputNode;
        } else if (i < temp){
            return SELECT(inputNode.getLeftChild(), i);
        } else {
            return SELECT(inputNode.getRightChild(), i - temp);
        }
    }
    public int keyRANK(redBlackTree inputTree, RedBlackNode inputKey){
        int temp = inputKey.leftChild.size + 1;
        RedBlackNode y = inputKey;
        while (y != inputTree.getRoot()) {
            if(y == y.getParent().getRightChild()){
                temp = temp + y.getParent().getLeftChild().getSize() + 1;
            }
            y = y.getParent();
        }
        return temp;
    }
    // Interval-Insert Node Red-Black Tree
    public RedBlackNode interval_Search(redBlackTree inputTree, Interval inputInterval){
        RedBlackNode temp = inputTree.getRoot();
        while (temp != null) {
            if(temp.getLeftChild() != null && temp.getLeftChild().max >= inputInterval.getLow()){
    ...
    public void rotationWithLeftChild(RedBlackNode inputNode) {
        if (inputNode != null) {
            ...
            tempNode.size = inputNode.getSize();
            inputNode.size = inputNode.getLeftChild().getSize() + inputNode.getRightChild().getSize() + 1;
        }
    }
